#ifndef DMM_SUBFLOW_H
#define DMM_SUBFLOW_H
...
struct SUBFLOW {
    ...
    int feedbackFD;
    int feedbackType;
    // for MPBond server to identify feedback on different paths
    int subflowId;

    int Setup(const char * localIP, const char * remoteIP, int feedbackType, int subflowId, int nSubflow);
