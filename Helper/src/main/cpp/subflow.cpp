...
int SUBFLOW::Setup(const char * localIP, const char * remoteIP, int feedbackType, int subflowId, int nSubflow) {
...
//    serverAddr.sin_port = htons((unsigned short) svrPort);
        ...
        if (connect(fd[i], (const struct sockaddr *) &serverAddr, sizeof(serverAddr)) != 0)
            return R_FAIL;
        LOGD("%d Helper's %d subflow connected to %s", n, i + 1, remoteIP);
        SetNonBlockIO(fd[i]);
        usleep(200000);
    }
...
void SUBFLOW::SendPIE() {
    ...
    buf[leng] = 0;
    *((int *) buf) = subflowId;
    *((int *)(buf + 4)) = kernelInfo.pipeBW[0];//5000;//thrpt//kernelInfo.pipeOWD[0];//netRTT;
    *((int *)(buf + 8)) = kernelInfo.pipeRTT[0];//10;//netRTT//kernelInfo.bytesInPipe[0];//bytesInPipe;
    *((int *)(buf + 12)) = kernelInfo.bytesInPipe[0];//1000;//bytesInPipe//kernelInfo.bytesOnDevice[0];//bytesOnDevice;
    *((unsigned long long *)(buf + 16)) = 23;
//    LOGI("subflow=%d, thrpt=%d", i, kernelInfo.pipeBW[i]);
    LOGI("[SendPIE] subflow=%d, throughput=%d, netRTT=%d, bytesInPipe=%d",
     subflowId, kernelInfo.pipeBW[0], kernelInfo.pipeRTT[0], kernelInfo.bytesInPipe[0]);
